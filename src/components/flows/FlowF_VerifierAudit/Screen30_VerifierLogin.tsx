  ...
  return (
    <div id="screen-30-container" className="min-h-[80vh] flex flex-col justify-center max-w-lg mx-auto py-8 px-4 space-y-6">
      ...
      <div className="text-center space-y-2">
        <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-[#6254E8]/10 border border-[#6254E8]/20 text-[#6254E8] mb-1 shadow-2xs">
          <ShieldCheck className="w-6 h-6" />
        </div>
        <div className="text-xs route-path text-[#8A8F98] font-medium">
          https://verify.tulacarbon.io/login
        </div>
        <h1 className="text-2xl font-semibold text-[#17181A] font-sans">
      ...
      <div className="p-4 bg-white border border-[#E5E7EB] rounded-xl text-xs space-y-2.5 shadow-2xs">
        <div className="font-semibold text-[#17181A] flex items-center justify-between font-sans">
          <div className="flex items-center space-x-1.5">
            <Info className="w-4 h-4 text-[#6254E8]" />
            <span>Assigned Verification Scope (FR-30.03 – FR-30.05)</span>
          </div>
          <StatusBadge status="Under Review" customLabel="Read-Only Scope" size="sm" />
        </div>
        <div className="grid grid-cols-2 gap-2 text-xs text-[#5F6368] pt-1 border-t border-[#F1F3F5]">
          <div>Tenant: <strong className="text-[#17181A] font-medium font-sans">{verifierScope.tenant}</strong></div>
          <div>Period: <strong className="text-[#17181A] period-code font-medium">{verifierScope.period}</strong></div>
          <div className="col-span-2">Sites: <strong className="text-[#17181A] font-medium font-sans">Chennai Plant 1, Pune Warehouse</strong></div>
        </div>
      </div>

      {/* Login Form (Section 21.1) */}
      <form onSubmit={handleSignIn} className="p-6 bg-white border border-[#E5E7EB] rounded-xl shadow-2xs space-y-4">
        <div>
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-3 py-2 text-xs border border-[#E5E7EB] rounded-lg focus:border-[#7567F5] focus:outline-none"
          />
        </div>
        ...
        <div className="pt-2">
          <button
            ...
            type="submit"
            disabled={isLoading}
            className="w-full h-10 enterprise-btn-primary text-xs font-semibold rounded-lg flex items-center justify-center space-x-2 shadow-xs disabled:opacity-50"
          >
      ...
      <div className="p-3.5 bg-[#FAFAFB] border border-[#E5E7EB] rounded-xl text-xs text-[#5F6368] space-y-1.5 shadow-2xs font-data">
        <div className="font-semibold text-[#17181A] font-sans">Verifier Access Constraints:</div>
      ...
      <div className="text-center">
        <button
          ...
          className="text-xs text-[#5F6368] hover:text-[#17181A] hover:underline font-sans"
        >
